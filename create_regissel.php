...
$user_type = 'seller'; // กำหนด user_type เป็น seller
...
if ($result->num_rows > 0) {
    ...
    if ($stmt->execute()) {
        ...
        if ($user_type === 'seller') {
            ...
            $sql = "SELECT MAX(seller_id) AS max_seller_id FROM sellers";
            $stmt = $conn->prepare($sql);
            if ($stmt === false) {
                die('Prepare failed: ' . htmlspecialchars($conn->error));
            }
            ...
            $last_seller_id = $row['max_seller_id'] ?? 0;
            $new_seller_id = max(3, $last_seller_id + 1); // เริ่มนับจาก 3

            // เพิ่มข้อมูลลงในตาราง sellers
            $sql = "INSERT INTO sellers (seller_id, user_id, seller_name) VALUES (?, ?, ?)";
            $stmt = $conn->prepare($sql);
            if ($stmt === false) {
                die('Prepare failed: ' . htmlspecialchars($conn->error));
            }
            $stmt->bind_param("iis", $new_seller_id, $user_id, $username);
